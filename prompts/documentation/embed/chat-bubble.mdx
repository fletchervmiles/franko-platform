...
| **Show only to logged-in users** | Wrap the snippet in your auth check:<br/>`{% if user.isAuthenticated %}`<br/>&nbsp;&nbsp;`<!-- snippet here -->`<br/>`{% endif %}` |
| **Show based on user role** | Use your template logic to conditionally render |

> The script loads asynchronously and is less than 5kb so it won't slow down your page.

**Want to identify users?** Add the identity snippet before your chat bubble script. [Learn more →](../identity-verification)
