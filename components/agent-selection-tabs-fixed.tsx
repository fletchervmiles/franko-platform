...
export default function AgentSelectionTabs({ 
  ...
  console.log('[AgentSelectionTabs-FIXED] Received organisationName prop:', organisationName);
  
  // Use internal state to manage the display name
  const [displayName, setDisplayName] = useState(organisationName);
  
  // Update internal state when prop changes
  useEffect(() => {
    console.log('[AgentSelectionTabs-FIXED] organisationName prop changed to:', organisationName);
    // Only update if the new value is meaningful
    if (organisationName && organisationName !== "your product") {
      console.log('[AgentSelectionTabs-FIXED] Updating display name to:', organisationName);
      setDisplayName(organisationName);
    }
  }, [organisationName]);
  
  // Log when rendering with current values
  useEffect(() => {
    console.log('[AgentSelectionTabs-FIXED] Rendering with displayName:', displayName);
  }, [displayName]);
  ...
  const agents = [
    ...
    {
      id: "onboard",
      title: "Customer Acqusition Agent",
      icon: UserPlus,
                ...
                {selectedAgent === "pmf" && (
                  <>
                    <li className="flex items-start">
                      ...
                      <span>{`How disappointed users would be if they could no longer use ${displayName}`}</span>
                    </li>
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                      <span>
                        {`Which type of users they believe get the most value from ${displayName} (and where`}
                        they fit)
                      </span>
                    </li>
                    <li className="flex items-start">
                    ...
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                      <span>{`The one improvement that would make ${displayName} indispensable, and why.`}</span>
                    </li>
                  </>
                )}
                {selectedAgent === "churn" && (
                  <>
                    ...
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                      <span>{`The goal they hoped ${displayName} would solve and where it fell short.`}</span>
                    </li>
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                      <span>
                        {`Who they are (role, company type) and how ${displayName} fit their workflow.`}
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                ...
                {selectedAgent === "onboard" && (
                  <>
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                      <span>
                        {`Where new users first discovered ${displayName} and what triggered their search.`}
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                      <span>
                        {`The challenge or goal that motivated them to seek a tool like ${displayName}.`}
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                      <span>
                        {`What convinced them to choose ${displayName} over alternatives and their early`}
                        impression.
                      </span>
                    </li>
                    <li className="flex items-start">
                      <span className="mr-1.5">•</span>
                      <span>
                        {`Who they are (role, company type) and how they expect ${displayName} to fit into`}
                        daily work.
                      </span>
                    </li>
                  </>
